...
void regions are the result of a fragmentation process produced along 
...
\begin{center}
  ...
  the nearest bulk void to each system of the \textit{IP} (background 
  histogram) and \textit{RIP} (scatter plot) samples for BDM catalogue. 
  Gray colorbar indicates the number of \textit{IP} systems in each bin, 
  whereas hot colorbar indicates the FA of the host cell of \textit{RIP} 
  ...
  indicate normed histograms of each single property for the \textit{IP}
  sample.}
...
of the fractional anisotropy of \textit{RIP} systems. For the V-web scheme, 
it can be noticed that \textit{RIP} systems embedded into low anisotropy 
...
close distances. While \textit{RIP} systems in higher FA values (yellow 
...
possible biases between the distance and volume distributions of pair 
