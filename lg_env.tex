...
noted that percolation (the largest void region volume) is completely 
reduced for $\lambda_{th} = 0.0$, so despite of the previously established 
$\lambda_{th}$ optimum values for each scheme, we shall use $\lambda_{th} 
= 0.0$ just for the detection of void regions. 



Due to the domination of the large scale visual impression by voids, it is 
inevitable the presence of the percolation phenomenon, so the current 
chosen threshold value for percolation is justified because in spite of 
voids are necessarily connected, we are just interested in detecting bulk 
void regions
