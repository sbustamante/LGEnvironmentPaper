...
pair. The second approach is complementary to the first one, and consists 
in calculating the relative distance of each halo or halo pair to the 
nearest void regions, thereby allowing linking the influence of bulk voids 
to physical properties of the samples.
...
each eigenvalue for the two defined web schemes and for the \textit{GH}, 
...
instance, for T-web scheme, the occupation fraction of the \textit{RIP} 
...
This along with that shown in the table \ref{tab:Samples_Size} allow 
concluding both halo schemes reproduce an equivalent mass function of the 
halos and a similar spatial distribution of them since the occupation 
values of each sample are approximately close. Now, regarding the 
differences between web schemes, from the distribution of the web 
eigenvalues it is clear that each scheme quantify the structure of the 
cosmic web in very different ways, which can be explained by how each 
scheme is built.
